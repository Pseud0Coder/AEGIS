    ...
    for(let i=enemies.length-1;i>=0;i--){
        ...
        if(d<e.radius+player.radius){
            ...
            if(game.deathLevel.active&&!game.deathLevel.shielded){
                const lost=rules.deathCollisionScoreLoss(game.score,{active:game.deathLevel.active,shielded:game.deathLevel.shielded});
                game.score=Math.max(0,game.score-lost);game.deathLevel.scoreLost+=lost;
...
function update(dt){
    ...
    for(let i=0;i<offsets.length;i++){
        ...
        const trailLimit=settings.reducedMotion?6:22;
        if(armTrails[i].length>trailLimit)armTrails[i].shift();
    }
    ...
    const enemySpeedMod=rules.enemySpeedMultiplier({
        overdrive:game.deathLevel.active&&game.deathLevel.type==='overdrive',
        timeSlow:timeslowTimer>0,
        shielded:game.deathLevel.shielded
    });
    
    // Spawn rate modifier
    const spawnRateMod=rules.chaosSpawnMultiplier({
        chaos:game.deathLevel.active&&game.deathLevel.type==='chaos',
        shielded:game.deathLevel.shielded
    });
    ...
    for(let i=floatingTexts.length-1;i>=0;i--){
        const t=floatingTexts[i];if(!settings.reducedMotion)t.y+=t.vy*dt;t.life-=dt;
    ...
    if(!settings.reducedMotion)for(let s of stars){s.x-=s.z*15*dt;if(s.x<0){s.x=W;s.y=Math.random()*H}}
    
    // Update HUD
    setText(ui.score,rules.parseHighScore(game.score));
    setText(ui.combo,game.combo);
    setText(ui.lives,game.lives);
    setText(ui.arms,game.arms);
    setText(ui.pulse,game.pulseLevel);
    setText(ui.shields,game.shields);
    const energyPct=Math.max(0,Math.min(100,Math.round(player.energy/player.maxEnergy*100)));
    setWidth(ui.energyFill,energyPct);
    setAriaValue(ui.energyBar,energyPct);
    if(game.pulseLevel>0){
        const pct=Math.max(0,Math.min(100,Math.round(game.pulseActive?100:(game.pulseTimer/game.pulseCooldown*100))));
        setWidth(ui.pulseFill,pct);
        setAriaValue(ui.pulseBar,pct);
    }
    ...
    if(game.fieryTimer>0)html+=`<div class="pwr-ind" style="color:#ff6600;border-color:#ff6600">FIERY ${game.fieryTimer.toFixed(1)}</div>`;
    ...
    if(ui.powerups.innerHTML!==html)ui.powerups.innerHTML=html;
}
...
function draw(){
    ...
    ctx.save();
    if(!settings.reducedMotion&&game.shake>0.5)ctx.translate((Math.random()-0.5)*game.shake,(Math.random()-0.5)*game.shake);
    ...
    if(game.deathLevel.warning>0){
        const p=settings.reducedMotion?0.5:Math.sin(game.time*15)*0.5+0.5;
    ...
    if(game.state==='playing'||game.state==='gameover'){
        ...
        // Draw arm trails
        for(let armIdx=0;armIdx<offsets.length;armIdx++){
            const fullTrail=armTrails[armIdx];
            const trail=settings.reducedMotion?fullTrail.slice(-8):fullTrail;
        ...
        for(let armIdx=0;armIdx<offsets.length;armIdx++){
            ...
            // Fiery particles
            if(game.fieryTimer>0&&!settings.reducedMotion){
                for(let k=0;k<2;k++){
                    const fa=Math.random()*Math.PI*2;const fs=Math.random()*60+20;
                    addParticle(new Particle(bx,by,Math.cos(fa)*fs,Math.sin(fa)*fs,'#ff6600',Math.random()*2+1,Math.random()*0.3+0.1));
                }
            }
        }
        ctx.shadowBlur=0;
        ...
        if(pVis){
            const pulse=settings.reducedMotion?1:1+Math.sin(player.pulseTime*4)*0.08;
            ...
            if(game.shields>0||game.deathLevel.shielded){
                ctx.strokeStyle='#4488ff';ctx.lineWidth=3;ctx.globalAlpha=settings.reducedMotion?0.5:0.4+Math.sin(game.time*3)*0.2;
            ...
            if(game.fieryTimer>0){
                ctx.strokeStyle='#ff6600';ctx.lineWidth=2;ctx.globalAlpha=settings.reducedMotion?0.4:0.3+Math.sin(game.time*6)*0.2;
        ...
        if(game.magnetTimer>0){
            ...
            for(let i=0;i<12;i++){
                const a=(i/12)*Math.PI*2+(settings.reducedMotion?0:game.time*2);
                ctx.beginPath();
        ...
        if(game.lives===1){
            const p=settings.reducedMotion?1:Math.sin(game.time*6)*0.5+0.5;
            ctx.strokeStyle=`rgba(255, 68, 136, ${p*0.4})`;ctx.lineWidth=30;
            ctx.strokeRect(0,0,W,H);
        }
        
        // Death level timer display
        if(game.state==='playing'&&game.deathLevel.active){
...
function setWidth(element,value){const width=`${value}%`;if(element.style.width!==width)element.style.width=width}
function setAriaValue(element,value){const text=String(value);if(element.getAttribute('aria-valuenow')!==text)element.setAttribute('aria-valuenow',text)}
function clearGameInput(){
    for(const key of Object.keys(keys))keys[key]=false;
    player.vx=0;player.vy=0;
    if(window.resetTouchControls)window.resetTouchControls();
}
function setGameplayUiPaused(paused){
    const transientContainers=[
        document.getElementById('deathWarningContainer'),
        document.getElementById('deathBannerContainer'),
        document.getElementById('thresholdContainer')
    ];
    if(paused){
        ...
        for(const container of transientContainers)container.classList.add('hidden');
    }else{
        ui.hud.classList.remove('hidden');
        ui.powerups.classList.remove('hidden');
        ui.energyBar.classList.remove('hidden');
        ui.energyLabel.classList.remove('hidden');
        ui.pulseBar.classList.toggle('hidden',game.pulseLevel<=0);
        for(const container of transientContainers)container.classList.remove('hidden');
    }
}
function pauseGame(){
    if(game.state!=='playing')return false;
    game.state='paused';clearGameInput();
    setGameplayUiPaused(true);
...
function resumeGame(){
    ...
    setGameplayUiPaused(false);
    document.getElementById('pauseBtn').classList.remove('hidden');
    initAudio();
    if(window.syncTouchControls)window.syncTouchControls();
    announce('Game resumed');
    document.getElementById('pauseBtn').focus({preventScroll:true});
    return true;
}
...
for(const button of document.querySelectorAll('.setting-toggle')){
    button.addEventListener('click',()=>{
        ...
        settings=rules.normalizeSettings({...settings,[name]:!settings[name]});
        applySettings();saveSettings();
        if(name==='reducedMotion'&&settings.reducedMotion){
            particles.length=0;
            for(const trail of armTrails)trail.splice(0,Math.max(0,trail.length-6));
        }
        if(name==='soundEnabled'){
            if(settings.soundEnabled&&game.state==='playing')initAudio();
